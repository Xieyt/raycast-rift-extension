...
import { getWindowsInFocusedWorkspace, focusWindow, checkRiftInstalled, type WindowWithWorkspace } from "./utils/rift";
...
export default function Command() {
  const [windows, setWindows] = useCachedState<WindowWithWorkspace[]>("windows-focused", []);
  const [isLoading, setIsLoading] = useCachedState<boolean>("loading-focused", true);
  ...
  useEffect(() => {
    async function loadWindows() {
      ...
      const fetchedWindows = await getWindowsInFocusedWorkspace();
  ...
  const workspaceName = windows[0]?.workspace_name || "";
  const navigationTitle = `Windows in Workspace: ${workspaceName}`;

  return (
    <List isLoading={isLoading} navigationTitle={navigationTitle} searchBarPlaceholder="Search windows...">
